...
def explore_intent():
    try:
        ...
        chat_history_str,chat_history_list = WeixinUtils.get_chat_history(max_messages=friend.contextSize)
        if not chat_history_str:
            return jsonify({"error": "No chat history available"}), 400

        # 4. 调用LLM获取意图分析，传入settings参数
        response = LLMUtils.get_llm_response("生成意图", chat_history_str, friend, settings)
...
def gen_reply():
    try:
        ...
        # 3. 获取微信聊天记录
        chat_history_str,chat_history_list = WeixinUtils.get_chat_history()
        if not chat_history_str:
            return jsonify({"error": "No chat history available"}), 400

        # 4. 调用LLM生成回复
        response = LLMUtils.get_llm_response_with_intent(
            "通用", user_intent, chat_history_str, friend, settings
        )

        #构建一个 ReplyModel对象
        reply_model = ReplyModel(
            wechatNickname=chat_name,
            replyList=response.reply_list,
            contextNum=len(chat_history_list),
            # chat_history_list 最后一条消息，如果数组为空，就赋值“暂无”
            lastMessage=chat_history_list[-1].content if chat_history_list else "暂无",
        )

        logger.debug("回复："+str(asdict(reply_model)))

        return jsonify(asdict(reply_model)), 200
